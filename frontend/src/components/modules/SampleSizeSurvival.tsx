...
export function SampleSizeSurvival() {
  ...
  const [result, setResult] = useState<ApiResponse<SurvivalResult> | null>(null);
  ...
  const run = async () => {
    ...
    try {
      const res = await api.sampleSizeSurvival(params as Record<string, unknown>) as ApiResponse<SurvivalResult>;
  ...
  const tableData = result?.result
    ? (result.result.numberOfSubjects ?? []).map((n, i) => ({
        Stage: i + 1,
        "Subjects": n,
        "Events": result.result!.numberOfEvents?.[i] ?? "-",
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-green-50 border border-green-200 p-3">
              <p className="text-xs text-green-600 font-medium">Max Subjects</p>
              <p className="text-2xl font-bold text-green-800">{Math.round(result.result.maxNumberOfSubjects)}</p>
            </div>
            <div className="rounded-lg bg-blue-50 border border-blue-200 p-3">
              <p className="text-xs text-blue-600 font-medium">Max Events</p>
              <p className="text-2xl font-bold text-blue-800">{Math.round(result.result.maxNumberOfEvents)}</p>
            </div>
          </div>
          <ResultTable data={tableData} caption="Sample Size & Events by Stage" />
